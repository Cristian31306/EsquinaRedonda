...
<body>
    ...
    <!-- CABECERA -->
    <div class="header">
        <img src="{{ public_path('favicon.png') }}" alt="Logo ParkiApp">
        <div class="header-text">
            <h1>{{ $settings['business_name'] ?? 'PARQUEADERO' }}</h1>
            <p>NIT: {{ $settings['business_nit'] ?? '000.000.000-0' }}</p>
            <p style="font-size: 7px; color: #cbd5e1;">Sistema de Gestión de Parqueaderos — ParkiApp</p>
        </div>
        ...
        <tr>
            <td>
                <div class="summary-card">
                    <p>Ingresos por Tickets</p>
                    <h3>{{ '$' . number_format($summary['tickets'], 0, ',', '.') }}</h3>
                </div>
            </td>
            <td>
                <div class="summary-card">
                    <p>Ingresos por Mensualidades</p>
                    <h3>{{ '$' . number_format($summary['memberships'], 0, ',', '.') }}</h3>
                </div>
            </td>
            <td>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            @else
                <tr>
                    <td colspan="5" style="text-align:center;color:#94a3b8;padding:20px;">No hay cobros de tickets
                        registrados en este periodo.</td>
                </tr>
            @endif
        </tbody>
        @if($payments->count() > 0)
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            @else
                <tr>
                    <td colspan="6" style="text-align:center;color:#94a3b8;padding:20px;">No hay mensualidades registradas
                        en este periodo.</td>
                </tr>
            @endif
        </tbody>
