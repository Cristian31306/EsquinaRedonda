...
const form = useForm({
    ...
    type: null,
    observation: '',
    stay_type: 'pending_selection', // Initial state to force explicit selection
});
...
// Determinar si es horario de "Día" o "Noche" (5 AM - 5 PM)
...
const submit = () => {
    ...
    if (!form.plate) {
        errorMessage.value = 'Por favor, ingrese la placa.';
        plateInput.value?.focus();
        return;
    }
    if (!form.type) {
        errorMessage.value = 'Por favor, seleccione una Categoría.';
        return;
    }
    if (form.stay_type === 'pending_selection') {
        errorMessage.value = 'Por favor, seleccione el Tipo de Estadía.';
        return;
    }

    form.post(route('tickets.store'), {
        onSuccess: () => {
            form.reset(); // Resetea todo el formulario
            form.stay_type = 'pending_selection'; // Volver al estado inicial
            plateInput.value?.focus();
        },
    });
};
        ...
        <div class="h-full flex flex-col items-center justify-center -mt-8 relative"> <!-- Compensate layouts padding -->
            ...
            <div v-if="page.props.membership_info && page.props.membership_info.days_left <= 7"
                ...
                <div class="flex items-center gap-4">
                    <div
                        class="w-10 h-10 bg-amber-500 text-white rounded-xl flex items-center justify-center shadow-lg shadow-amber-200">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-amber-900 uppercase tracking-widest">Aviso de Vencimiento
                        </p>
                        <p class="text-xs font-bold text-amber-700">La mensualidad de este vehículo vence en <span
                                class="font-black underline">{{ page.props.membership_info.days_left }} días</span> ({{
                                    page.props.membership_info.end_date }}).</p>
                    </div>
                </div>
            ...
            <div
                class="bg-white border-2 border-slate-100 rounded-[3rem] p-8 max-w-4xl w-full shadow-[0_30px_60px_rgba(0,0,0,0.03)] animate-in slide-in-from-bottom-4 duration-500 overflow-hidden">

                <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                    <!-- Left Column: Input and Header -->
                    <div class="space-y-6 text-center lg:text-left">
                        <div>
                            <h1 class="text-xs font-black text-indigo-950 uppercase tracking-[0.3em] mb-1">Registro de
                                Ingreso
                            </h1>
                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Ingrese la placa
                                del
                                vehículo</p>
                        </div>

                        <div class="relative group">
                            <input ref="plateInput" v-model="form.plate" @input="form.plate = form.plate.toUpperCase()"
                                type="text" placeholder="AAA000"
                                ...
                                required />
                            <div
                                class="h-1.5 w-full bg-indigo-950 rounded-full mt-2 transition-all group-focus-within:w-full w-24 mx-auto lg:mx-0">
                            </div>
                        </div>

                        <div class="space-y-2">
                            <label
                                class="block text-[8px] font-black text-slate-400 uppercase tracking-widest px-1">Notas
                                Rápidas (Ej. Ralladura)</label>
                    ...
                    <div
                        class="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 flex flex-col justify-between h-full space-y-8">
                        <div class="space-y-4">
                            <p
                                class="text-center lg:text-left text-[9px] font-black text-slate-400 uppercase tracking-widest px-2">
                                Seleccione Categoría</p>
                            <div class="grid grid-cols-2 gap-2">
                                <button v-for="cat in categories" :key="cat" type="button" @click="setType(cat)"
                        ...
                        <div class="space-y-2 pt-2 border-t border-slate-200">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                ...
                                <button type="button" @click="form.stay_type = 'overnight'" :disabled="isDaytime"
                                    :class="[
                                        form.stay_type === 'overnight' ? 'bg-indigo-950 text-white border-indigo-950 scale-[1.05] shadow-lg shadow-indigo-100' : 'bg-white text-slate-500 border-slate-100 hover:bg-slate-50',
                                        isDaytime ? 'opacity-20 grayscale pointer-events-none' : ''
                                    ]"
                                    class="py-3 px-2 rounded-2xl border-2 font-black text-[8px] uppercase tracking-widest transition-all flex flex-col items-center gap-1.5 relative">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="2.5" stroke="currentColor" class="w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    Noche
                                    <span v-if="isDaytime"
                                        class="absolute -top-1 -right-1 bg-slate-900 text-[6px] text-white px-1.5 py-0.5 rounded-full uppercase">Indisponible</span>
                                </button>
                                <button type="button" @click="form.stay_type = 'fullday'" :disabled="!isDaytime" :class="[
                                    form.stay_type === 'fullday' ? 'bg-indigo-950 text-white border-indigo-950 scale-[1.05] shadow-lg shadow-indigo-100' : 'bg-white text-slate-500 border-slate-100 hover:bg-slate-50',
                                    !isDaytime ? 'opacity-20 grayscale pointer-events-none' : ''
                                ]"
                                    class="py-3 px-2 rounded-2xl border-2 font-black text-[8px] uppercase tracking-widest transition-all flex flex-col items-center gap-1.5 relative">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        ...
                        <button type="submit" :disabled="form.processing"
                            class="w-full py-5 bg-emerald-600 text-white font-black uppercase text-xs rounded-2xl shadow-xl shadow-emerald-100 hover:bg-emerald-700 active:scale-95 transition-all tracking-[0.2em] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
